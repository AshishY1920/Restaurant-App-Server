...
// Get Restaurants By Categories
exports.getRestaurantByCategories = async (req, res) => {
  try {
    ...
    if (!category) {
      return res.status(400).json({
        status: 0,
        message: 'Categories Is Required Field',
      });
    }
    const Restaurant = await RestaurantModel.find({categories: {$in: category}})
      .limit(Limit)
      .skip(skip);
